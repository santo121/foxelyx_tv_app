...
class _LoginPageState extends State<LoginPage> {
  ...
  bool _isLogoReady = false;
  bool _logoPrecacheStarted = false;
  String? _resolvedDeviceIdentifier;
  ...
  void didChangeDependencies() {
    ...
    precacheImage(
      const AssetImage('assets/logo/foxelyx_logo.png'),
      context,
    ).whenComplete(() {
      if (!mounted) return;
      setState(() {
              ...
              return RepaintBoundary(
                child: BlocListener<AuthCubit, AuthState>(
                  listener: (BuildContext context, AuthState state) {
                    if (state is AuthMacLoaded) {
                      _resolvedDeviceIdentifier = state.deviceIdentifier;
                    }
                    if (state is AuthAuthenticated) {
                      widget.onLoginSuccess();
                      return;
                    }
                    if (_resolvedDeviceIdentifier != null && _isLogoReady) {
                      FocusScope.of(context).requestFocus(_pairingCodeFocus);
                    }
                  },
                  child: BlocBuilder<AuthCubit, AuthState>(
                    builder: (BuildContext context, AuthState state) {
                      final String? deviceIdentifier =
                          _resolvedDeviceIdentifier;
                      final bool initialReady =
                          deviceIdentifier != null && _isLogoReady;
                      if (!initialReady) {
                        if (state is AuthError && deviceIdentifier == null) {
                          return _LoginBootstrapError(
                            message: state.message,
                            onRetry: () =>
                                context.read<AuthCubit>().loadMacAddress(),
                          );
                        }
                        return const _LoginBootstrapLoading();
                      }
                      final bool loading = state is AuthLoading;
                      final String? error = state is AuthError
                          ? state.message
                          : null;
                      ...
                      final Widget formColumn = Column(
                        ...
                        children: <Widget>[
                          ...
                          Text(
                            ...
                            style: TextStyle(
                              color: Colors.white,
                              fontSize: (screenSize.shortestSide * 0.055).clamp(
                                22.0,
                                40.0,
                              ),
                      ...
                      return Padding(
                        ...
                        child: Row(
                          crossAxisAlignment: CrossAxisAlignment.stretch,
                          children: <Widget>[
                            Expanded(flex: 5, child: Center(child: logoColumn)),
...
class _LoginBootstrapLoading extends StatelessWidget {
  const _LoginBootstrapLoading();
...
class _LoginBootstrapError extends StatelessWidget {
  const _LoginBootstrapError({required this.message, required this.onRetry});
